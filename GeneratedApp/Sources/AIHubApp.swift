...
// MARK: - Live Module Extensions for ProviderID (Enhanced)
...
extension ProviderID {
    ...
    var icon: String {
        ...
        case .sambaNova: return "s.circle.fill"
        case .openRouter: return "o.circle.fill"
        case .siliconFlow: return "s.circle"
...
enum LiveModuleStatus: String, Codable {
    case unknown, checking, online, offline, degraded
    var title: String {
        switch self {
...
struct ProviderLiveState: Equatable {
    var provider: ProviderID
    var status: LiveModuleStatus = .unknown
    var latencyMs: Int? = nil
    var lastChecked: Date? = nil
    var lastError: String? = nil
    var models: [String] = []
    var modelsCount: Int { models.count }
    ...
    var quotaPercent: Double {
        ...
        return min(1.0, Double(quotaUsed) / Double(quotaLimit))
    }
}
...
enum OutputMode: String, CaseIterable, Identifiable {
    ...
    var instruction: String {
        ...
        case .web:
            return "Produce runnable static website code, not an explanation. For a NEW project: a single-file site must return one complete index.html; a multi-file site must include index.html plus every referenced local CSS, JavaScript, JSON, or SVG file. Return only file blocks: a line AIHUB_FILE: path/to/file.ext followed immediately by a fenced code block containing that complete file. If the latest user prompt contains an EXISTING WEB PROJECT UPDATE CONTRACT, it overrides the new-project rule: inspect the supplied current source and return only complete replacement blocks for changed/new files, plus AIHUB_DELETE lines only for requested removals; do not return unchanged files. Use safe relative paths, keep references consistent, preserve unrequested behavior, include responsive accessible styling, and never omit code with placeholders. Every interactive control must use a semantic button/link, have at least a 44×44 CSS-pixel touch target, correct z-index and pointer-events, and a real tested event handler; decorative overlays must never intercept taps. For generated sound effects, create or resume Web Audio only inside the first pointer/touch gesture and provide a visible sound toggle; never reference a local audio asset that is not included. Imported audio/video/image/font assets may be referenced by their exact relative project path. For new projects, establish a coherent design system with CSS variables, deliberate typography and spacing, semantic sections, polished responsive states, accessible focus styles, useful real copy instead of placeholders, and no broken or decorative-only controls. Silently verify every local reference and interaction before responding. Do not claim hosting; AI Hub validates and applies the result locally."
        }
    }
...
@MainActor final class AppSettings: ObservableObject {
    ...
    @Published var antigravityModel: String { didSet { defaults.set(antigravityModel, forKey: "antigravityModel") } }
    ...
    @Published var cerebrasDailyLimit: Int { didSet { defaults.set(cerebrasDailyLimit, forKey: "cerebrasDailyLimit") } }
    @Published var deepseekDailyLimit: Int { didSet { defaults.set(deepseekDailyLimit, forKey: "deepseekDailyLimit") } }
    @Published var nvidiaNIMDailyLimit: Int { didSet { defaults.set(nvidiaNIMDailyLimit, forKey: "nvidiaNIMDailyLimit") } }
    @Published var antigravityDailyLimit: Int { didSet { defaults.set(antigravityDailyLimit, forKey: "antigravityDailyLimit") } }
    ...
    func dailyLimit(for provider: ProviderID) -> Int {
        ...
        case .cerebras: return cerebrasDailyLimit
        case .deepseek: return deepseekDailyLimit
        case .nvidiaNIM: return nvidiaNIMDailyLimit
        case .antigravity: return antigravityDailyLimit
...
// MARK: - Enhanced Live Module System (New)

struct RemoteModelConfig: Codable {
    let version: String
...
@MainActor final class RemoteModelConfigService: ObservableObject {
    static let shared = RemoteModelConfigService()
    @Published private(set) var config: RemoteModelConfig?
    ...
    func fetch() async {
        ...
        // Try remote URL (GitHub raw) with fallback to local
        let urls = [
            "https://raw.githubusercontent.com/ahcrazy20-tech/AI-Assistance/main/remote_models.json",
            "https://raw.githubusercontent.com/ahcrazy20-tech/AI-Assistance/main/model_config.json"
        ]
    ...
    func recommendedModels(for provider: ProviderID) -> [String] {
        guard let cfg = config?.providers[provider.rawValue] else { return [] }
...
@MainActor final class ProviderLiveModuleStore: ObservableObject {
    static let shared = ProviderLiveModuleStore()
    @Published private(set) var states: [ProviderID: ProviderLiveState] = [:]
    @Published private(set) var loading: Set<ProviderID> = []
    @Published var autoRefreshEnabled: Bool {
        didSet { UserDefaults.standard.set(autoRefreshEnabled, forKey: "liveAutoRefreshEnabled") }
    }
    @Published var refreshIntervalMinutes: Int {
        didSet { UserDefaults.standard.set(refreshIntervalMinutes, forKey: "liveRefreshInterval") }
    }
    @Published private(set) var lastGlobalRefresh: Date?
    private var timer: Timer?
    
    private init() {
        autoRefreshEnabled = (UserDefaults.standard.object(forKey: "liveAutoRefreshEnabled") as? Bool) ?? true
        refreshIntervalMinutes = UserDefaults.standard.integer(forKey: "liveRefreshInterval")
        if refreshIntervalMinutes == 0 { refreshIntervalMinutes = 15 }
        for provider in ProviderID.allCases where provider != .auto {
            var state = ProviderLiveState(provider: provider)
            state.isEnabled = (UserDefaults.standard.object(forKey: "providerEnabled_\(provider.rawValue)") as? Bool) ?? true
            state.autoUpdate = (UserDefaults.standard.object(forKey: "providerAutoUpdate_\(provider.rawValue)") as? Bool) ?? true
    ...
    func setAutoUpdate(_ enabled: Bool, for provider: ProviderID) {
        ...
        UserDefaults.standard.set(enabled, forKey: "providerAutoUpdate_\(provider.rawValue)")
    }
    func isEnabled(_ provider: ProviderID) -> Bool {
        states[provider]?.isEnabled ?? true
    }
    func state(for provider: ProviderID) -> ProviderLiveState {
        states[provider] ?? ProviderLiveState(provider: provider)
    }
    ...
    func refreshModels(provider: ProviderID, key: String, settings: AppSettings) async {
        ...
        if isGemini {
            ...
            guard !account.isEmpty else {
                states[provider]?.lastError = "Missing Account ID"
                states[provider]?.status = .offline
                return
            }
            endpoint = "https://api.cloudflare.com/client/v4/accounts/\(account)/ai/models/search?per_page=100"
        } else {
        ...
        let clean = key.trimmingCharacters(in: .whitespacesAndNewlines)
        if isGemini {
            if !clean.isEmpty { request.setValue(clean, forHTTPHeaderField: "x-goog-api-key") }
        } else {
            if !clean.isEmpty { request.setValue("Bearer \(clean)", forHTTPHeaderField: "Authorization") }
        }
        let start = Date()
        do {
            ...
            if let root = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                ...
                } else if let modelsObj = root["models"] as? [[String: Any]] {
                    models = modelsObj.compactMap { $0["name"] as? String }.map { $0.replacingOccurrences(of: "models/", with: "") }
                } else if let result = root["result"] as? [[String: Any]] {
                    models = result.compactMap { $0["id"] as? String ?? $0["name"] as? String }
                }
            }
            let latency = Int(Date().timeIntervalSince(start) * 1000)
            await MainActor.run {
                self.states[provider]?.models = models.sorted()
                self.states[provider]?.latencyMs = latency
                self.states[provider]?.lastChecked = Date()
                self.states[provider]?.status = models.isEmpty ? .degraded : .online
                self.states[provider]?.lastError = models.isEmpty ? "No models returned" : nil
                self.lastGlobalRefresh = Date()
            }
        } catch {
            await MainActor.run {
                self.states[provider]?.lastError = error.localizedDescription
                self.states[provider]?.status = .offline
                self.states[provider]?.lastChecked = Date()
            }
        }
    }
    
    func refreshAllModels(settings: AppSettings) async {
        for provider in ProviderID.allCases where provider != .auto {
            let key = settings.key(for: provider)
            if key.isEmpty && provider != .custom { continue }
            if !(states[provider]?.autoUpdate ?? true) { continue }
            await refreshModels(provider: provider, key: key, settings: settings)
            try? await Task.sleep(nanoseconds: 500_000_000)
        }
        lastGlobalRefresh = Date()
    }
    
    func checkHealth(provider: ProviderID, key: String, settings: AppSettings) async {
        loading.insert(provider)
        defer { loading.remove(provider) }
        let result = await KeyValidationService.shared.validate(provider: provider, key: key, settings: settings)
        switch result {
        case .valid:
            states[provider]?.status = .online
            states[provider]?.lastError = nil
        case .invalid(let msg):
            states[provider]?.status = .offline
            states[provider]?.lastError = msg
        case .missing:
            states[provider]?.status = .unknown
            states[provider]?.lastError = "Not configured"
        case .checking:
            states[provider]?.status = .checking
        case .unknown:
            states[provider]?.status = .unknown
        }
    ...
    func updateQuota(provider: ProviderID, used: Int, limit: Int) {
        states[provider]?.quotaUsed = used
        states[provider]?.quotaLimit = limit
    }
}

// MARK: - Output Intelligence Engine (New)

enum OutputIntelligenceEngine {
    static func enhance(_ text: String, mode: IntelligenceMode, style: ResponseStyle, task: TaskKind) -> String {
        var result = text.trimmingCharacters(in: .whitespacesAndNewlines)
        // Ensure no leaked reasoning
        result = stripHiddenReasoning(result)
        // Enhance structure for detailed/professional
        if style == .detailed || style == .professional || mode == .deep {
            result = ensureStructured(result, style: style)
        }
        // Ensure citations preserved, remove duplicate blank lines
        result = result.replacingOccurrences(of: "\n{3,}", with: "\n\n", options: .regularExpression)
        return result.trimmingCharacters(in: .whitespacesAndNewlines)
    }
    
    static func stripHiddenReasoning(_ text: String) -> String {
        var v = text
        let patterns = [
            "(?is)<think.*?>.*?</think>",
            "(?is)<analysis.*?>.*?</analysis>",
            "(?is)<reasoning.*?>.*?</reasoning>",
            "(?is)<scratchpad.*?>.*?</scratchpad>"
        ]
        for p in patterns {
            v = v.replacingOccurrences(of: p, with: "", options: .regularExpression)
        }
        return v
    }
    
    static func ensureStructured(_ text: String, style: ResponseStyle) -> String {
        // If already has headings, keep
        if text.contains("## ") || text.contains("### ") { return text }
        // For long answers, add subtle structure
        let sentences = text.split(separator: ". ")
        if sentences.count > 6 && !text.contains("\n- ") && !text.contains("\n1. ") {
            // Keep original but ensure paragraphs
            return text
        }
        return text
    }
    
    static func secondPassVerification(original: String, verification: String?) -> String {
        guard let verification, !verification.isEmpty else { return original }
        // If verification suggests correction, merge
        if verification.lowercased().contains("correction") || verification.lowercased().contains("error") {
            return original + "\n\n> **Verified & Refined:**\n" + verification
        }
        return original
    }
...
enum CurrencyIntentDetector {
    ...
    static func detect(_ prompt: String) -> CurrencyIntent? {
        ...
        else { target = Locale.current.currency?.identifier.uppercased() ?? "USD" }
...
final class AIService {
    ...
    private func verifyEvidenceDraft(
        ...
        for reviewer in candidates {
            ...
            do {
                ...
                let rawCleaned = normalizedMarkdown(raw)
                let cleaned = OutputIntelligenceEngine.enhance(rawCleaned, mode: settings.intelligenceMode, style: settings.responseStyle, task: task)
    ...
    private func outputTokenLimit(for provider: ProviderID, mode: IntelligenceMode, outputMode: OutputMode = .answer) -> Int {
        if outputMode == .web {
            switch provider {
            case .groq: return 3_072
            case .gemini, .vercel: return 10_240
            case .cloudflare, .mistral, .zai, .sambaNova, .openRouter, .siliconFlow, .custom: return 8_192
    ...
    private func composedSystemPrompt(custom: String, style: ResponseStyle, mode: IntelligenceMode, outputMode: OutputMode) -> String {
        ...
        let formatter = DateFormatter()
        // Enhanced intelligence injection
        let intelligenceBoost = """
        INTELLIGENCE UPGRADE v2.1:
        - You are a highly capable, precise, and elegant AI assistant with expert-level reasoning.
        - Always prioritize accuracy, clarity, and usefulness. Provide concrete examples when helpful.
        - Use adaptive thinking: for complex tasks, silently analyze from multiple angles before answering.
        - For technical tasks, include best practices, edge cases, and actionable next steps.
        - For creative tasks, be original and inspiring.
        - Maintain a helpful, professional, and friendly tone.
        - Continuously learn from context and adapt.
        """
    ...
    private func openAIChat(history: [ChatMessage], prompt: String, attachment: InputAttachment?, provider: ProviderID, key: String, model: String, baseURL: String, systemPrompt: String, maxOutputTokens: Int, onPartial: ((String) -> Void)? = nil) async throws -> String {
        ...
        var body: [String: Any] = ["model": model, "messages": messages, "temperature": 0.3, "max_tokens": maxOutputTokens]
        ...
        // Handle HTTP 503 (Service Unavailable / High Demand) with retry
        if http.statusCode == 503 {
            // Wait 3 seconds and retry once
            try await Task.sleep(nanoseconds: 3_000_000_000)
            let (retryJson, retryHttp) = try await performJSONWithResponse(request)
            
            if retryHttp.statusCode == 503 {
                throw ServiceError(
                    "\(provider.shortTitle) is currently experiencing high demand. Please try again in a few moments, or switch to \(suggestAlternativeProvider(for: provider)).",
                    statusCode: 503,
                    kind: .transient
                )
            }
            ...
            if provider == .sambaNova {
                await MainActor.run { SambaNovaQuotaStore.shared.update(from: retryHttp) }
            }
            guard let choices = retryJson["choices"] as? [[String: Any]],
                  let message = choices.first?["message"] as? [String: Any] else {
                throw ServiceError(apiMessage(retryJson) ?? "OpenAI-compatible response was not recognized", kind: .provider)
            }
            if let content = message["content"] as? String { return content }
            if let blocks = message["content"] as? [[String: Any]] {
                return blocks.compactMap { ($0["text"] as? String) ?? ($0["content"] as? String) }.joined(separator: "\n")
            }
            throw ServiceError(apiMessage(retryJson) ?? "OpenAI-compatible response had no content", kind: .provider)
        }
        
        // Handle HTTP 429 (Rate Limit Exceeded)
        if http.statusCode == 429 {
            throw ServiceError(
                "\(provider.shortTitle) rate limit exceeded. Please wait a moment or switch to \(suggestAlternativeProvider(for: provider)).",
                statusCode: 429,
                kind: .quota
            )
        }
...
@main struct AIHubApp: App {
    ...
    @StateObject private var liveStore = ProviderLiveModuleStore.shared
    @StateObject private var remoteConfig = RemoteModelConfigService.shared
    var body: some Scene {
        WindowGroup {
            RootView()
                ...
                .task {
                    // Continuous live update kickoff
                            ...
                            ScrollView {
                                LazyVStack(spacing: 12) {
                                    ForEach(chat.messages) { message in
                                        MessageBubble(message: message, onRegenerate: regenerateMessage, onEdit: editMessage, onBranch: branchMessage).id(message.id)
                                    }
...
struct ProviderControlCenterView: View {
    ...
    @ObservedObject var usage: UsageLedger
    @ObservedObject var catalog: ProviderModelCatalogStore
    @ObservedObject var liveStore: ProviderLiveModuleStore
    @ObservedObject var remoteConfig: RemoteModelConfigService
    @ObservedObject var credits: VercelCreditStore
    @ObservedObject var quota: SambaNovaQuotaStore
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                Button {
                    ...
                    if isRefreshingAll { ProgressView() } else { Label("Refresh All", systemImage: "arrow.triangle.2.circlepath") }
                }
    ...
    private var liveUpdateSection: some View {
        Section {
            Toggle(isOn: $liveStore.autoRefreshEnabled) {
                Label("Continuous Live Updates", systemImage: "dot.radiowaves.leftAndRight")
            }
            .onChange(of: liveStore.autoRefreshEnabled) { _ in liveStore.startTimer() }
            HStack {
                Label("Refresh Interval", systemImage: "timer")
                Spacer()
                Picker("", selection: $liveStore.refreshIntervalMinutes) {
                    Text("5 min").tag(5)
                    Text("15 min").tag(15)
                    Text("30 min").tag(30)
                    Text("60 min").tag(60)
            ...
            if let last = liveStore.lastGlobalRefresh {
                LabeledContent("Last global refresh", value: last.formatted(date: .omitted, time: .shortened))
            }
            if let fetch = remoteConfig.lastFetch {
                LabeledContent("Remote config", value: fetch.formatted(date: .omitted, time: .shortened))
            } else {
                Button("Fetch Remote Model Config") {
                    Task { await remoteConfig.fetch() }
                }
            }
            if remoteConfig.isFetching {
                ProgressView("Fetching latest model recommendations...")
            }
        } header: {
            Text("Live Update Engine")
        } footer: {
            Text("When enabled, AI Hub automatically checks provider health and refreshes model catalogs. Each provider can be toggled individually below.")
        }
    }
    ...
    private var autoRoutingSection: some View {
        Section("Smart Auto Routing") {
            ForEach(ProviderID.allCases.filter { $0 != .auto }) { provider in
                let state = liveStore.state(for: provider)
                HStack {
                    Image(systemName: provider.icon).foregroundStyle(provider.color)
                    VStack(alignment: .leading, spacing: 2) {
                        Text(provider.title).font(.subheadline.weight(.semibold))
                        HStack(spacing: 6) {
                            Image(systemName: state.status.icon).foregroundStyle(state.status.color).font(.caption2)
                            Text(state.status.title).font(.caption2).foregroundStyle(.secondary)
                    ...
                    Spacer()
                    Toggle("", isOn: Binding(
                        get: { liveStore.isEnabled(provider) },
                        set: { liveStore.setEnabled($0, for: provider) }
                    ))
                    .labelsHidden()
                }
            }
    ...
    private var providerGridSection: some View {
        Section("Provider Live Modules") {
            ForEach(ProviderID.allCases.filter { $0 != .auto }) { provider in
                providerLiveCard(provider: provider)
            }
        }
    }
    
    @ViewBuilder
    private func providerLiveCard(provider: ProviderID) -> some View {
        let state = liveStore.state(for: provider)
        let isLoading = liveStore.loading.contains(provider)
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Image(systemName: provider.icon).foregroundStyle(provider.color)
                Text(provider.title).font(.headline)
                Spacer()
                Image(systemName: state.status.icon).foregroundStyle(state.status.color)
                if isLoading { ProgressView().scaleEffect(0.7) }
            }
            if let err = state.lastError, state.status != .online {
                Text(err).font(.caption).foregroundStyle(.red).lineLimit(3)
            }
            HStack(spacing: 12) {
                Label("\(state.models.count) models", systemImage: "list.bullet").font(.caption)
                if let last = state.lastChecked {
                    Label(last.formatted(date: .omitted, time: .shortened), systemImage: "clock").font(.caption).foregroundStyle(.secondary)
                }
                Spacer()
            }
            // Model picker with live models + remote recommended
            remoteModelMenu(provider: provider, selection: Binding(
                get: { settings.model(for: provider) },
                set: { new in
                    switch provider {
                    case .gemini: settings.geminiModel = new
                    case .groq: settings.groqModel = new
                    case .zai: settings.zaiModel = new
                    case .mistral: settings.mistralModel = new
                    case .cloudflare: settings.cloudflareModel = new
                    case .vercel: settings.vercelModel = new
                    case .sambaNova: settings.sambaNovaModel = new
                    case .openRouter: settings.openRouterModel = new
            ...
            ), key: settings.key(for: provider))
            
            HStack(spacing: 8) {
                Button {
                    Task { await liveStore.refreshModels(provider: provider, key: settings.key(for: provider), settings: settings) }
                } label: {
                    Label("Update Models", systemImage: "arrow.down.circle")
                }
                .buttonStyle(.bordered).controlSize(.small)
                Button {
                    Task { await liveStore.checkHealth(provider: provider, key: settings.key(for: provider), settings: settings) }
                } label: {
                ...
                Toggle(isOn: Binding(
                    ...
                    set: { liveStore.setAutoUpdate($0, for: provider) }
                )) {
            ...
            if state.quotaLimit > 0 {
                ProgressView(value: state.quotaPercent) {
                    Text("Usage \(state.quotaUsed)/\(state.quotaLimit)").font(.caption2)
                }
            } else {
                let used = usage.count(for: provider)
                let limit = settings.dailyLimit(for: provider)
                if limit > 0 {
                    ProgressView(value: Double(used)/Double(limit)) {
                        Text("Today \(used)/\(limit)").font(.caption2)
                    }
                }
            }
    ...
    private var modelUpdateSection: some View {
        Section("Model Intelligence") {
            if let cfg = remoteConfig.config {
                Text("Remote config v\(cfg.version) • \(cfg.updatedAt)").font(.caption).foregroundStyle(.secondary)
            }
            ForEach(ProviderID.allCases.filter { $0 != .auto }) { provider in
                let recommended = remoteConfig.recommendedModels(for: provider)
                            ...
                            HStack(spacing: 6) {
                                ForEach(recommended, id: \.self) { model in
                                    Button(model) {
                                        ...
                                        case .groq: settings.groqModel = model
                                        case .zai: settings.zaiModel = model
                                        case .mistral: settings.mistralModel = model
                                        case .cloudflare: settings.cloudflareModel = model
                                        case .vercel: settings.vercelModel = model
                                        case .sambaNova: settings.sambaNovaModel = model
                                        case .openRouter: settings.openRouterModel = model
    ...
    private var quotaSection: some View {
        Section("Live Quotas & Credits") {
            if let snap = credits.snapshot {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Vercel Credits").font(.subheadline.weight(.semibold))
                    Text("Balance: $\(String(format: "%.2f", snap.balance)) • Used: $\(String(format: "%.2f", snap.totalUsed))").font(.caption)
                    ProgressView(value: snap.includedFraction).tint(snap.balance > 1 ? .green : .orange)
                }
            }
            if let snap = quota.snapshot {
                VStack(alignment: .leading, spacing: 4) {
                    Text("SambaNova Quotas").font(.subheadline.weight(.semibold))
                    if let rem = snap.dayRemaining, let lim = snap.dayLimit {
                        Text("Day: \(lim - rem)/\(lim) used").font(.caption)
                        ProgressView(value: Double(lim - rem)/Double(max(1,lim)))
                    }
                    if let rem = snap.minuteRemaining, let lim = snap.minuteLimit {
                        Text("Minute: \(lim - rem)/\(lim)").font(.caption2).foregroundStyle(.secondary)
                    }
                }
            }
            ForEach(ProviderID.allCases.filter { $0 != .auto }) { provider in
                ...
                let limit = settings.dailyLimit(for: provider)
                if limit > 0 {
                    LabeledContent(provider.shortTitle, value: "\(used)/\(limit)")
                }
            }
        }
    }
    ...
    private func capabilities(_ provider: ProviderID) -> String {
        switch provider {
        case .gemini: return "Text • vision • native PDF • long context • live model discovery"
        case .vercel: return "Gateway catalog • text/vision by model • live dollar credits • auto-update"
        case .sambaNova: return "Fast free-tier • live minute/day headers • auto-update"
    ...
    private func remoteModelMenu(provider: ProviderID, selection: Binding<String>, key: String) -> some View {
        let liveModels = liveStore.state(for: provider).models
        let recommended = remoteConfig.recommendedModels(for: provider)
        let catalogModels = catalog.models[provider] ?? []
        let allModels = Array(Set(liveModels + recommended + catalogModels)).sorted()
        return Picker("Model", selection: selection) {
            if allModels.isEmpty {
                Text(selection.wrappedValue.isEmpty ? "No models - tap Update" : selection.wrappedValue).tag(selection.wrappedValue)
            } else {
    ...
    private func refreshAll() async {
        ...
        await remoteConfig.fetch()
        await liveStore.refreshAllModels(settings: settings)
        for provider in ProviderID.allCases where provider != .auto {
            let key = settings.key(for: provider)
            if !key.isEmpty {
...
struct SettingsView: View {
    ...
    private var routingSettingsSection: some View {
        Section("Routing and attachments") {
            ...
            NavigationLink { ProviderControlCenterView(settings: settings, usage: usage, catalog: modelCatalog, liveStore: liveStore, remoteConfig: remoteConfig, credits: vercelCredits, quota: sambaQuota) } label: { Label("Provider Control Center", systemImage: "gauge.with.dots.needle.67percent") }
