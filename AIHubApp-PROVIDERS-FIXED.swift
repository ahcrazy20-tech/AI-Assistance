...
enum ProviderID: String, CaseIterable, Identifiable, Codable, Hashable {
    case auto, gemini, groq, zai, mistral, cloudflare, vercel, sambaNova, openRouter, siliconFlow, cerebras, githubModels, deepseek, nvidiaNIM, custom
    ...
    var title: String {
        ...
        case .githubModels: return "GitHub Models — GPT-4o free"
...
@MainActor final class AppSettings: ObservableObject {
    ...
    @Published var cerebrasKey: String { didSet { defaults.set(cerebrasKey, forKey: "cerebrasKey") } }
    @Published var cerebrasModel: String { didSet { defaults.set(cerebrasModel, forKey: "cerebrasModel") } }
    @Published var githubModelsKey: String { didSet { defaults.set(githubModelsKey, forKey: "githubModelsKey") } }
    @Published var githubModelsModel: String { didSet { defaults.set(githubModelsModel, forKey: "githubModelsModel") } }
    @Published var deepseekKey: String { didSet { defaults.set(deepseekKey, forKey: "deepseekKey") } }
    @Published var deepseekModel: String { didSet { defaults.set(deepseekModel, forKey: "deepseekModel") } }
    @Published var nvidiaNIMKey: String { didSet { defaults.set(nvidiaNIMKey, forKey: "nvidiaNIMKey") } }
    ...
    init() {
        ...
        cerebrasKey = store.string(forKey: "cerebrasKey") ?? ""
        cerebrasModel = store.string(forKey: "cerebrasModel") ?? "llama3.1-70b"
        githubModelsKey = store.string(forKey: "githubModelsKey") ?? ""
        githubModelsModel = store.string(forKey: "githubModelsModel") ?? "gpt-4o"
        deepseekKey = store.string(forKey: "deepseekKey") ?? ""
        deepseekModel = store.string(forKey: "deepseekModel") ?? "deepseek-chat"
        nvidiaNIMKey = store.string(forKey: "nvidiaNIMKey") ?? ""
    ...
    func dailyLimit(for provider: ProviderID) -> Int {
        ...
        case .githubModels: return 150  // 150 req/day
...
final class AIService {
    ...
    private func providerCandidates(hasVisual: Bool, isDocument: Bool, mode: IntelligenceMode, outputMode: OutputMode = .answer) -> [ProviderID] {
        if outputMode == .web {
            ...
            return [.vercel, .gemini, .deepseek, .githubModels, .cerebras, .openRouter, .zai, .custom, .mistral, .sambaNova, .siliconFlow, .cloudflare, .groq, .nvidiaNIM]
        }
        if hasVisual || isDocument {
            // Native PDF and real vision providers lead; text-only providers remain late OCR/text fallbacks.
            return [.gemini, .vercel, .zai, .githubModels, .cloudflare, .openRouter, .mistral, .sambaNova, .custom, .groq, .siliconFlow, .cerebras, .deepseek, .nvidiaNIM]
        }
        switch mode {
        case .fast: return [.groq, .cerebras, .sambaNova, .zai, .gemini, .mistral, .cloudflare, .siliconFlow, .openRouter, .custom, .vercel, .deepseek, .githubModels, .nvidiaNIM]
        case .smart: return [.gemini, .zai, .sambaNova, .groq, .cerebras, .mistral, .cloudflare, .siliconFlow, .openRouter, .custom, .vercel, .deepseek, .githubModels, .nvidiaNIM]
        case .deep: return [.vercel, .gemini, .deepseek, .githubModels, .zai, .sambaNova, .mistral, .cloudflare, .openRouter, .groq, .siliconFlow, .custom, .cerebras, .nvidiaNIM]
        case .research: return [.gemini, .vercel, .deepseek, .zai, .sambaNova, .mistral, .cloudflare, .openRouter, .groq, .siliconFlow, .custom, .cerebras, .githubModels, .nvidiaNIM]
        case .agent: return [.vercel, .gemini, .deepseek, .zai, .sambaNova, .mistral, .cloudflare, .openRouter, .groq, .siliconFlow, .custom, .cerebras, .githubModels, .nvidiaNIM]
        case .compare: return [.vercel, .gemini, .deepseek, .githubModels, .zai, .sambaNova, .mistral, .groq, .cloudflare, .openRouter, .siliconFlow, .custom, .cerebras, .nvidiaNIM]
        }
    }
    ...
    private func contextBudget(for provider: ProviderID, mode: IntelligenceMode) -> Int {
        ...
        case .githubModels: base = 100_000  // GPT-4o has 128K context
    ...
    private func outputTokenLimit(for provider: ProviderID, mode: IntelligenceMode, outputMode: OutputMode = .answer) -> Int {
        if outputMode == .web {
            ...
            case .githubModels: return 12_288  // GPT-4o output
    ...
    private func supportsVisualInput(provider: ProviderID, model: String) -> Bool {
        ...
        case .cerebras, .deepseek, .nvidiaNIM:
            return visualMarkers.contains(where: value.contains)
        case .githubModels: return value.contains("gpt-4") || value.contains("gpt-5") || visualMarkers.contains(where: value.contains)
...
final class KeyValidationService {
    ...
    func validate(provider: ProviderID, key: String, settings: AppSettings) async -> KeyCheckState {
        ...
        case .groq, .zai, .mistral, .vercel, .sambaNova, .siliconFlow, .custom,
             .cerebras, .githubModels, .deepseek, .nvidiaNIM:
            var base = settings.baseURL(for: provider).trimmingCharacters(in: CharacterSet(charactersIn: "/"))
            if base.hasSuffix("/chat/completions") { base = String(base.dropLast("/chat/completions".count)) }
...
struct ProviderControlCenterView: View {
    ...
    private func capabilities(_ provider: ProviderID) -> String {
        ...
        case .githubModels: return "GPT-4o/5 free • GitHub PAT auth • 150 req/day"
...
struct SettingsView: View {
    ...
    private var alternativeProviderSections: some View {
        ...
        providerSection(provider: .githubModels, title: "GitHub Models — GPT-4o free", key: $githubModelsKey, model: $settings.githubModelsModel, limit: .constant(150))
